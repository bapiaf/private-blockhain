...
class BlockchainController {
    ...
    // This endpoint allows you to trigger the validation of the blockchain 
    validateChain() {
        this.app.get("/validateChain", async (req, res) => {
            
                try {
                    let chainValidation = await this.blockchain.validateChain();
                    if(chainValidation){
                        return res.status(200).json(chainValidation);
                    } else {
                        return res.status(500).send("Could not validate the chain");
                    }
                } catch (error) {
                    return res.status(500).send("Could not validate the chain");
                }
